...
Reservoir::Reservoir(size_t R, size_t range, size_t L, size_t total){
...
void Reservoir::add(int label, int *hashes, std::string fastaEntry, std::ofstream& outputFasta){
    /*
...
#pragma omp parallel for
    ...
    if (averageCount/_R < _L) {
        _sample.push_back(make_pair(label, fastaEntry));
        _labelSet.insert(label);
        
        outputFasta << fastaEntry;
    }
}
